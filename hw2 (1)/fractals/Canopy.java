...
{
   /**
      Create a fractal canopy tree in the xy-plane with
      {@code n} branches and starting with the line segment
      (the tree's "trunk") from {@code (0.0, -1.0, 0.0)}
      to {@code (0.0, -0.5, 0.0)}.
                     ...
                     double angle, final int n)
   {
      ...
      if (n > 0)
      {
         ...
         addVertex(new Vertex(
                    ((1+Math.sqrt(2))*x1 - x0 - Math.tan(Math.PI/360*angle) * (y0 - y1))/Math.sqrt(2),
